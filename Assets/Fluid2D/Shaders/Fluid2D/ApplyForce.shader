Shader "Hidden/GPGPU/Fluid2D/ApplyForce" {
	...
	//uniform sampler2D _cursorClipSpace;
	//uniform sampler2D _LastcursorClipSpace;

uniform	float4 _Pforce[1000];
uniform float4 _Lastforce[1000];
	...
	float4 frag (v2f_img i) : SV_Target
	{
		...
		for (int j = 0; j < 1000; j++) {
			...
			float2 mouse = _Pforce[j].xy;
			float2 lastMouse = _Lastforce[j].xy;
			...
			float taperFactor = 0.8;//1 => 0 at lastMouse, 0 => no tapering
